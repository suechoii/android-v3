...
@AndroidEntryPoint
class TotalCareerFragment : BaseFragment<FragmentUnivtotalPageBinding>(R.layout.fragment_univtotal_page) {
    ...
    override fun onResume() {
        ...
        if (MingleApplication.pref.isUpdate) {
            if (firstPosition != 0) {
                viewModel.getTotalNextPosts(3, tempLastPostId)
            } else {
                isFirst = true
                viewModel.getTotalList(3, true)
            }
        }
    }
    ...
    private fun initViewModel() {
        ...
        viewModel.getTotalList(3, false)
        ...
        viewModel.univTotalList3.observe(binding.lifecycleOwner!!) {
            if (it == null) {
        ...
        viewModel.lastPostId3.observe(binding.lifecycleOwner!!) {
            lastPostId = it
            if (lastPostId != -1)
                tempLastPostId = lastPostId
        }

        viewModel.newUnivTotalList3.observe(binding.lifecycleOwner!!) {
            totalListAdapter.addUnivTotalList(it, isFirst)
            binding.swipeRefresh.isRefreshing = false
            currentPostList = it.toTypedArray()
        }
        ...
        binding.univtotalRv.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                if (!binding.univtotalRv.canScrollVertically(1) && lastPosition == totalCount && lastPostId != -1) {
                    viewModel.getTotalNextPosts(3, lastPostId)
                }
            }
        })
    }
