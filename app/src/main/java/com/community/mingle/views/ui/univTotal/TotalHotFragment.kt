...
@AndroidEntryPoint
class TotalHotFragment : BaseFragment<FragmentUnivtotalPageBinding>(R.layout.fragment_univtotal_page) {
    ...
    override fun onResume() {
        ...
        if (MingleApplication.pref.isUpdate) {
            if (firstPosition != 0) {
                viewModel.getTotalNextBestPosts(tempLastPostId)
            } else {
                isFirst = true
                viewModel.getTotalBestPosts(true)
            }
        }
    }
    ...
    private fun initViewModel() {
        ...
        viewModel.getTotalBestPosts( false)
        ...
        binding.univtotalRv.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                if (!binding.univtotalRv.canScrollVertically(1) && lastPosition == totalCount && lastPostId != -1) {
                    viewModel.getTotalNextBestPosts(lastPostId)
                }
            }
        })
    }
