...
@AndroidEntryPoint
class TotalNewsFragment : BaseFragment<FragmentUnivtotalPageBinding>(R.layout.fragment_univtotal_page) {
    ...
    override fun onResume() {
        ...
        if (MingleApplication.pref.isUpdate) {
            if (firstPosition != 0) {
                viewModel.getTotalNextPosts(4, tempLastPostId)
            } else {
                isFirst = true
                viewModel.getTotalList(4, true)
            }
        }
    }
    ...
    private fun initViewModel() {
        ...
        viewModel.getTotalList(4, false)
