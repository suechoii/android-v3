...
@AndroidEntryPoint
class UnivCareerFragment : BaseFragment<FragmentUnivtotalPageBinding>(R.layout.fragment_univtotal_page) {
    ...
    override fun onResume() {
        ...
        if (MingleApplication.pref.isUpdate) {
            if (firstPosition != 0) {
                viewModel.getUnivNextPosts(3, tempLastPostId)
            } else {
                isFirst = true
                viewModel.getUnivList(3, true)
            }
        }
        ...
        binding.univtotalRv.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                if (!binding.univtotalRv.canScrollVertically(1) && lastPosition == totalCount && lastPostId != -1) {
                    viewModel.getUnivNextPosts(3, lastPostId)
                }
            }
        })
    }
