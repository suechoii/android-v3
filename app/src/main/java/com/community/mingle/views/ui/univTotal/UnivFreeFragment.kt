...
@AndroidEntryPoint
class UnivFreeFragment : BaseFragment<FragmentUnivtotalPageBinding>(R.layout.fragment_univtotal_page) {
    ...
    override fun onResume() {
        ...
        if (MingleApplication.pref.isUpdate) {
            if (firstPosition != 0) {
                Log.d("what to do","ah")
                viewModel.getUnivNextPosts(1, tempLastPostId)
            } else {
                Log.d("what to do","ah2")
                isFirst = true
                viewModel.getUnivList(1, true)
            }
